...
from benchmark_db import parse_directory_flow, parse_directory_rgb, parse_directory_rgb1, parse_split_file
from utils.video_funcs import default_aggregation_func
from action_caffe import CaffeNet

parser = argparse.ArgumentParser()
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51', 'kinetics'])
parser.add_argument('split', type=int, choices=[1, 2, 3], help='on which split to test the network')
...
parser.add_argument('--num_frame_per_video', type=int, default=25, help="prefix of y direction flow images")
...
def eval_video(video):
    ...
    if np.argmax(default_aggregation_func(frame_scores)) == label:
        ii = 1
    print 'video {0} pred {1} label {2} same {3} done'.format(vid, np.argmax(default_aggregation_func(frame_scores)),
                                                              label, ii)
    sys.stdin.flush()
